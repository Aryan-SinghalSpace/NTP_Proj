...
Two top-level folders keep docs and code cleanly separated:
- **`docs/`** — all documentation, the UI mockups (`docs/mockups/`), and git-ignored confidential reference material (`docs/reference/`).
- **`code/`** — the actual application monorepo (run all `pnpm` commands from here).
- Root keeps only `CLAUDE.md` (auto-loaded), `README.md`, `.gitignore`.

## Codebase (`code/`, scaffold v0, 2026-06-21)

Monorepo (pnpm + Turborepo) — **run from `code/`** (e.g. `pnpm -C code dev`). See `docs/SETUP.md`.
- `code/apps/api` — NestJS modular monolith. RLS-aware `TenantDbService`, tenant-context middleware, `/health`, `/fields`. Drizzle ORM + raw-SQL migrations in `code/apps/api/drizzle`.
- `code/apps/web` — Next.js 14 App Router on the Command × Bento Tailwind tokens. Real **Dashboard** page (bento KPIs / event-volume chart / recent events / recall / FEFO) built with **mock data**; Field Library page reads the API when it's up. Reusable shell (`components/TopNav`, `components/icons`) + mock-data layer (`lib/mock`).
- `code/packages/field-types` — shared Zod field-type system + GS1 validators (strict typing, both sides).
- `code/infra/` — docker-compose (Postgres + RLS app role, Redis, Temporal + UI, MinIO).
...
> **Confidential reference (git-ignored, local-only):** `docs/reference/Hakuna Matata/` and `docs/reference/DK-2-Prod-Features--main/` are third-party DataKart material — study as reference, **never clone, never commit/push**.
