...
/* ── live-API → UI mapping ─────────────────────────────────────
   The API returns raw product columns; the existing UI expects a richer shape
   (a colour swatch, formatted identity, pack hierarchy, batches). Batches and
   deeper pack levels have no backend yet, so we derive display-only fields and
   leave those empty until their slices exist. */

const TONES: Tone[] = ['primary', 'teal', 'amber', 'rose', 'sky', 'violet'];

/** Deterministic swatch from the UUID so a product keeps its colour across loads. */
function swatchFor(seed: string): Tone {
  let h = 0;
  for (let i = 0; i < seed.length; i++) h = (h * 31 + seed.charCodeAt(i)) >>> 0;
  return TONES[h % TONES.length] ?? 'primary';
}

function toUiProduct(p: ApiProduct): Product {
  const a = p.attributes ?? {};
  const each: PackLevel = {
    level: 'Each',
    scheme: 'GTIN',
    code: p.gtin ?? '— assigned at commit —',
    qty: '1 unit',
  };
  return {
    id: p.id,
...
export default function MasterDataClient({ initialProducts }: { initialProducts: ApiProduct[] }) {
  ...
  const committed = products.filter((p) => p.status === 'committed');
  const drafts = products.filter((p) => p.status === 'draft');
  const [hierId, setHierId] = useState<string | null>(committed[0]?.id ?? null);
  ...
  const hierProduct = committed.find((p) => p.id === hierId) ?? committed[0] ?? null;
    ...
    <>
      ...
      <main className="mx-auto max-w-[1180px] px-6 py-7">
        ...
        <div className="mb-6 flex flex-wrap items-end gap-3">
          <div>
            ...
            <p className="mt-1 text-[13.5px] text-muted">
              Acme Foods · <span className="font-semibold text-teal">live from the API through Postgres RLS</span> ·
              products keyed by internal UUID · GTIN is a validated attribute
            </p>
          </div>
          <div className="ml-auto flex gap-2.5">
            <Btn icon={<DownloadIcon width={16} height={16} />} onClick={() => flash(`Export queued — ${products.length} products (CSV)`)}>
              Export
            </Btn>
        ...
        {tab === 'hierarchy' && (
          <HierarchyTab
            committed={committed}
            productId={hierProduct?.id ?? null}
            setProductId={setHierId}
            product={hierProduct}
          />
        )}
      ...
      {creating && (
        <CreateProductModal
          ...
          onCreated={(name) => {
            setCreating(false);
            flash(`Draft product “${name}” created`);
            router.refresh(); // re-fetch the server component → live list updates
          }}
...
/* ── Create product modal (writes through POST /api/products) ── */
...
function CreateProductModal({
  ...
  const set = (k: keyof CreateProductPayload) => (e: React.ChangeEvent<HTMLInputElement>) =>
    setForm((f) => ({ ...f, [k]: e.target.value }));

  const required: (keyof CreateProductPayload)[] = ['brand', 'name', 'netContent', 'packType', 'brandOwner', 'category'];
  const valid = required.every((k) => String(form[k] ?? '').trim().length > 0);
    ...
    <div className="fixed inset-0 z-50">
      ...
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-[440px] flex-col overflow-auto border-l border-border bg-surface shadow-lg">
        <div className="flex items-center gap-3 border-b border-border p-5">
          ...
          <div>
            <div className="font-display text-[17px] font-bold">New product</div>
            <div className="text-[12px] text-muted">Created as a draft — GTIN is assigned at commit</div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col gap-3.5 p-5">
          <FormField label="Product name" value={form.name} onChange={set('name')} placeholder="e.g. Choco Bar 50g" />
          <div className="grid grid-cols-2 gap-3.5">
            <FormField label="Brand" value={form.brand} onChange={set('brand')} placeholder="Velvet" />
            <FormField label="Category" value={form.category} onChange={set('category')} placeholder="Confectionery" />
          </div>
          <div className="grid grid-cols-2 gap-3.5">
            <FormField label="Net content" value={form.netContent} onChange={set('netContent')} placeholder="50 g" />
            <FormField label="Pack type" value={form.packType} onChange={set('packType')} placeholder="Flow wrap" />
          </div>
          <FormField label="Brand owner" value={form.brandOwner} onChange={set('brandOwner')} placeholder="Acme Foods Pvt Ltd" />
          <FormField label="MRP (optional)" value={mrp} onChange={(e) => setMrp(e.target.value)} placeholder="₹20.00" />
          <p className="text-[11.5px] text-subtle">
            Persists through <span className="font-mono">POST /api/products</span> with the tenant scoped by RLS —
            the server never trusts a tenant id from the body.
          </p>
        </div>
...
function HierarchyTab({
  ...
  return (
    <div className="rounded-3xl border border-border bg-surface p-6">
      ...
      {!product ? (
        <p className="text-[13px] text-muted">No committed products yet — commit a draft to build its pack hierarchy.</p>
      ) : (
      ...
      <p className="mt-5 text-[12.5px] text-subtle">
        Aggregation events (Each → Case → Pallet) build this hierarchy; disaggregation unwinds it. Deeper pack
        levels arrive with the events slice.
      </p>
    </div>
  );
}
